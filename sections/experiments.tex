...
rate than questions with zero entropy. Using behavioral uncertainty (defined in \autoref{sec:methods:signals}), we split
correct-at-$T_0$ questions into a \textbf{certain} group ($H = 0$, all 25
samples agree) and an \textbf{uncertain} group ($H < 0$, samples spread
...
\textbf{less} effective on uncertain questions (a pattern we do not
observe).
