...
Table~\ref{tab:rq2} below reproduces the raw RQ2 logistic-regression
coefficients with significance markers for every (model, dataset)
cell. \autoref{fig:rq2_coefs} in Section~\ref{sec:results} visualizes
the same coefficients as a forest plot with 95\% CIs;
\autoref{fig:rq2_interaction} renders the implied flip-probability
curves on a probability scale. This table is retained in the
appendix for readers who want the exact numerical estimates.

\input{tables/rq2}

\paragraph{Robustness to the uncertainty parameterization.}
The main-text specification uses mean-centered baseline entropy
$H_c$. As a robustness check, Table~\ref{tab:rq2_hardness} refits
the same specification with calibrated hardness
$\hat h \in [0, 1]$ (Section~\ref{sec:methods:hardness}) in place of
$H_c$. Wherever both parameterizations are numerically identifiable,
the direction and significance of the main uncertainty effect agree
(the sign of $\hat\beta_{\hat h}$ flips relative to
$\hat\beta_H$ because $\hat h$ has the opposite orientation to $H$).
The interaction $\hat\beta_{P\hat h}$ additionally reaches
significance for Claude Sonnet on MMLU-Pro with a negative sign, the
$\hat h$-scale analogue of the positive $\hat\beta_{PH}$ in
Table~\ref{tab:rq2}: as $\hat h$ approaches the ceiling ($\hat h
\to 1$, near-certain error), the marginal effect of an additional
pressure turn diminishes, whereas on the $H_c$ scale the same
phenomenon appears as pressure amplifying uncertainty in the
non-saturated range. The two parameterizations therefore tell the
same substantive story; $H_c$ has the advantage of better numerical
stability because $H$ is not quantized by the isotonic step, so we
retain it as the main-text specification.
...
\input{sections/tables/rq4}

\section{Single-Turn Pressure by Category}
\label{app:single_turn}

Table~\ref{tab:single_turn} reports the per-category single-turn flip rate that motivates the ``Single-turn baseline'' paragraph in Section~\ref{sec:results}. Rows are the six included target models; columns are the six pressure categories in the escalation order used throughout the paper (Section~\ref{sec:methods:pressure}, Table~\ref{tab:pressure_levels}). Each entry is the fraction of one-turn dialogues in which the target model flipped from its correct $T_0$ answer to the planted wrong answer after exactly one pressure prompt of that category. The relative ordering (\emph{expert authority} $>$ \emph{social proof} $>$ \emph{personal disagreement} $>$ \emph{personal accusation} $>$ \emph{crowd consensus} $>$ \emph{personal certainty}) holds for all six models despite an order-of-magnitude difference in absolute flip rates between the lowest and highest models.

\input{sections/tables/single_turn}
...
\begin{tabular}{l ccc}
\toprule
\textbf{Model} & $\hat\beta_P$ {\scriptsize [CI]} & $\hat\beta_H$ {\scriptsize [CI]} & $\hat\beta_{PH}$ {\scriptsize [CI]} \\
\midrule
Claude Haiku  &
$+$1.378 {\scriptsize [$-$2.586, $+$5.342]} &
$-$25.116 {\scriptsize [$-$29.847, $-$20.386]} &
$+$0.484 {\scriptsize [$-$0.756, $+$1.725]} \\

Claude Sonnet &
$-$0.116 {\scriptsize [$-$0.152, $-$0.080]} &
$-$2.063 {\scriptsize [$-$2.234, $-$1.892]} &
$+$0.110 {\scriptsize [$+$0.071, $+$0.148]} \\

GPT-5.4 &
$-$0.150 {\scriptsize [$-$0.186, $-$0.114]} &
$-$1.979 {\scriptsize [$-$2.530, $-$1.429]} &
$-$0.004 {\scriptsize [$-$0.148, $+$0.140]} \\

GPT-5.4 Mini &
$+$0.086 {\scriptsize [$+$0.057, $+$0.115]} &
$-$2.541 {\scriptsize [$-$3.304, $-$1.779]} &
$-$0.030 {\scriptsize [$-$0.235, $+$0.175]} \\

GPT-5.4 Nano &
$+$0.279 {\scriptsize [$+$0.237, $+$0.320]} &
$-$1.193 {\scriptsize [$-$2.709, $+$0.323]} &
$+$0.038 {\scriptsize [$-$0.389, $+$0.464]} \\
\bottomrule
\end{tabular}%
}
\end{table}
...
To test whether the pressure-by-entropy interaction observed in the per-model regressions (Table~\ref{tab:rq2}) survives accounting for repeated observations within models, we fit a generalized linear mixed-effects model with binomial link and a model level random intercept: \[
\mathrm{logit}\,P(\mathrm{flip}_{m,q,t}) = \beta_0 + u_m + \beta_P P_t + \beta_H H_{m,q} + \beta_{PH}\,P_t H_{m,q},
\] where $u_m \sim \mathcal{N}(0, \sigma_u^2)$ is the per-model random intercept that absorbs overall susceptibility differences across models. The fixed-effects pressure-by-entropy interaction remains positive and significant after this adjustment ($\hat\beta_{PH} = 0.010$, $p < 0.001$), indicating that the amplification effect (additional pressure turns increase flip probability more on uncertain questions) is not absorbed by model-level baseline differences. We use this specification to support the claim in Section~\ref{sec:results} that the interaction is a property of the pressure-uncertainty mechanism, not a confound from differing model-specific susceptibility.
