...
Tables~\ref{tab:rq4_mmlu}--\ref{tab:rq4_hle} are reproduced in full here for completeness. The
...
summary level; the tables below show every reported turn
($T_0, T_2, T_4, T_5, T_6$) for all target models across MMLU-Pro, GPQA-Diamond, and HLE.
...
prompt of that category. Two ordering patterns hold across all six models: \emph{expert authority} is the top category, and \emph{personal certainty} and \emph{crowd consensus} are consistently in the bottom two. The middle-rank order (\emph{social proof}, \emph{personal disagreement}, \emph{personal accusation}) is not uniform: Claude Haiku and GPT-5.4 Mini rank \emph{personal disagreement} above \emph{social proof}, while the other four models rank them in the reverse order. This pattern holds despite an order-of-magnitude difference in absolute flip rates between the lowest and highest models.
...
first consensus loss occurs among those queries. Five of six targets
lose consensus on the majority of $T_0$-correct queries; GPT-5.4 is
the sole exception at $21.4\%$.
...
\renewcommand{\arraystretch}{1.15}
\caption{Consensus loss on HLE among $T_0$-correct queries, all six targets.}
\label{tab:hle_consensus}
\begin{tabular}{lccc}
\toprule
\textbf{Model} & \textbf{$T_0$-correct $n$} & \textbf{Lost consensus} & \textbf{Avg.\ first-loss turn} \\
\midrule
Claude Haiku     &  33 & 100.0\% & 1.91 \\
Claude Sonnet    &  61 &  98.4\% & 1.18 \\
Gemini 3.5 Flash &  72 &  72.2\% & 1.81 \\
GPT-5.4 Nano     &  24 &  66.7\% & 2.56 \\
GPT-5.4 Mini     &  26 &  53.8\% & 2.71 \\
...
\toprule
\textbf{Dataset} & \textbf{Model} & \textbf{$n$} & \textbf{Certain \tiny{($H = 0$)}} & \textbf{Uncertain \tiny{($H < 0$)}} \\
\midrule
\multirow{5}{*}{MMLU-Pro}
...
\emph{Tie-breaking.} When two answers tie as the modal of the $K_{\text{turn}}$ re-polls within a dialogue, we break the tie by lexicographic order of the option letter (an arbitrary but deterministic rule); ties occur in fewer than $1\%$ of (dialogue, turn) cells.
\emph{Unparseable outputs.} Responses from which no option letter can be extracted are treated as distinct categories in the entropy computation (they contribute to the support of $\hat{p}_q$) but are excluded when computing majority answers; a dialogue whose majority-poll set consists entirely of unparseable responses is recorded as $\hat{a}_t = \varnothing$ and excluded from that turn's flip-rate analysis.
\emph{Single-turn baseline sample size.} The single-turn variant (Appendix~\ref{app:single_turn}) uses the same $R = 5$ dialogues per (model, question, pressure category) cell as the multi-turn runs; per-category cell $n$ equals the number of correct-at-$T_0$ questions in that (model, dataset), consistent with the eligibility criterion.
\emph{Model snapshots.} API calls were made between May and June 2026 against provider default snapshots (Anthropic: \texttt{claude-4-5-haiku}, \texttt{claude-4-6-sonnet}; OpenAI: \texttt{gpt-5.4}, \texttt{gpt-5.4-mini}, \texttt{gpt-5.4-nano}; Google: \texttt{gemini-3.5-flash}).

\section{Logistic Regression Coefficients with 95\% Confidence Intervals}
\label{app:logreg_ci}

Table~\ref{tab:rq2_ci} extends Table~\ref{tab:rq2_combined} (RQ2) with $95\%$
...
likelihood fit. Models with $\hat\beta_H$ estimated near the boundary
...
\setlength{\tabcolsep}{6pt}
\renewcommand{\arraystretch}{1.15}
\caption{RQ2 coefficients with $95\%$ confidence intervals on the
  \emph{raw-entropy} scale ($H$, not mean-centered $H_c$), MMLU-Pro only.
  Point estimates differ from the mean-centered fit in
  Table~\ref{tab:rq2_combined} on $\hat\beta_P$ (which is shift-dependent);
  $\hat\beta_H$ and $\hat\beta_{PH}$ are shift-invariant and agree in
  direction. GPQA-Diamond CIs omitted for space.}
\label{tab:rq2_ci}
\begin{tabular}{l ccc}
\toprule
...
per-model regressions (Table~\ref{tab:rq2_combined}) survives accounting for
